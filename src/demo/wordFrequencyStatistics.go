...
func updateFrequencies(filename string, frequencyForWord map[string]int)  {
	...
	if file, err = os.Open(filename); err != nil{
		log.Print("failed to open the file: ", err)
		return
	}
	defer file.Close()
...
func readAndUpdateFrequencies(reader *bufio.Reader, frequencyForWord map[string]int)  {
	for {
		...
		//strings.TrimSpace 删除这一行字符串的前面和后面的空格
		for _, word := range SplitOnNonLetters(strings.TrimSpace(line)) {
			//SplitOnNonLetters 返回仅是字母的字符串
...
func SplitOnNonLetters(s string)[]string {
	...
	//创建一个匿名函数 notALetter,如果传入的是字符就返回 false， 如果不是字符就返回 true
